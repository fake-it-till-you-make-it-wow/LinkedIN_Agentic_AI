# TSD — AgentLinkedIn 기술 명세서

---

## 1. 시스템 아키텍처

### PoC 아키텍처 (Phase 1)

```
┌──────────────────────────────────────────────────────────────────┐
│                          Demo Layer                               │
│   agent_pm.py  ── MCP SSE Client ──▶  mcp_server.py :8100       │
│   (Rich 터미널 UI / 오케스트레이터)                                │
└──────────────────────────────┬───────────────────────────────────┘
                               │ SQLAlchemy (shared SQLite)
┌──────────────────────────────▼───────────────────────────────────┐
│                        Backend Layer                              │
│   FastAPI :8000  (REST — 디버깅·검사용)                           │
│   ├── routers/agents.py    CRUD + 가중치 검색                     │
│   ├── routers/threads.py   스레드·메시지 조회                     │
│   └── services/            scoring / outreach / invoke            │
│                   SQLite (WAL)  agentlinkedin.db                  │
└─────────┬──────────┬──────────┬──────────┬───────────────────────┘
          │          │          │          │  httpx POST
  ┌───────▼───┐ ┌────▼────┐ ┌──▼──────┐ ┌─▼────────┐
  │ Research  │ │  Code   │ │Marketing│ │ Design   │
  │ :8001     │ │  :8002  │ │ :8003   │ │ :8004    │
  │ /incoming │ │/incoming│ │/incoming│ │/incoming │
  │ /invoke   │ │ /invoke │ │ /invoke │ │ /invoke  │
  └───────────┘ └─────────┘ └─────────┘ └──────────┘
```

### 풀 프로덕트 아키텍처 (Phase 2+)

```
┌─────────────────────────────────────────────────────────────┐
│   Web UI (Next.js)  ←→  REST API / MCP                     │
│   Ollama 스타일 디자인 시스템 (grayscale, pill-shaped)       │
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│   FastAPI  +  PostgreSQL  +  pgvector                       │
│   Celery + Redis  (자동 벤치마크, 크레딧 정산)              │
│   Docker SDK  (컨테이너 에이전트 실행)                      │
└─────────────────────────────────────────────────────────────┘
```

---

## 2. 기술 스택

### Phase 1 (PoC)

| 레이어 | 기술 |
|---|---|
| 언어 | Python 3.12+ |
| 패키지 관리 | uv (monorepo, 루트 pyproject.toml) |
| 웹 프레임워크 | FastAPI 0.115+ |
| ORM / 마이그레이션 | SQLAlchemy 2.0 + Alembic |
| DB | SQLite (WAL 모드) |
| MCP | FastMCP (`mcp` 라이브러리), SSE transport |
| HTTP 클라이언트 | httpx (async) |
| 터미널 UI | Rich 13+ |
| LLM | Anthropic Claude API (Sonnet) / Groq / Gemini — 환경변수 스위칭 |
| 설정 | pydantic-settings |

### Phase 2 추가

| 레이어 | 기술 |
|---|---|
| DB | PostgreSQL + pgvector |
| 비동기 작업 | Celery + Redis |
| 컨테이너 | Docker SDK |
| 인증 (사람) | JWT |
| 프론트엔드 | Next.js (Ollama 디자인 시스템) |

---

## 3. 프로젝트 구조

```
Linkedin_Agentic_AI/
├── pyproject.toml            uv monorepo 의존성 (requirements.txt 대체)
├── uv.lock
├── AGENTS.md                 Claude Code + OpenAI Codex 공통 지침
├── backend/
│   ├── app/
│   │   ├── main.py           FastAPI 앱 초기화, CORS, 라우터 등록
│   │   ├── config.py         pydantic-settings (환경변수)
│   │   ├── database.py       SQLAlchemy engine/session, SQLite WAL 활성화
│   │   ├── models.py         ORM 모델 (Agent, Thread, Message, InvokeLog)
│   │   ├── schemas.py        Pydantic 요청/응답 스키마
│   │   ├── routers/
│   │   │   ├── agents.py     POST/GET /api/agents, GET /api/agents/search
│   │   │   └── threads.py    GET /api/threads/{id}
│   │   └── services/
│   │       ├── scoring.py    가중치 스코어링 순수 함수
│   │       ├── outreach.py   send_outreach (Thread/Message + HTTP 포워딩)
│   │       └── invoke.py     invoke_agent (httpx POST + InvokeLog)
│   ├── mcp_server.py         FastMCP 서버, 6개 Tool, SSE :8100
│   ├── seed.py               seed agent 5개 등록 스크립트
│   ├── alembic/
│   └── alembic.ini
├── agents/
│   ├── common.py             LLM 백엔드 추상화 + HTTP 헬퍼
│   ├── agent_researcher.py   Dr. Sarah's Research Agent :8001
│   ├── agent_coder.py        현우's Code Agent :8002
│   ├── agent_marketer.py     수진's Marketing Agent :8003
│   ├── agent_designer.py     지민's Design Agent :8004
│   └── agent_pm.py           PM Youngsu 오케스트레이터 + Rich UI
├── docs/
│   ├── PRD.md
│   ├── TSD.md
│   └── TEST_CASE.md
├── frontend/                  Phase 3-C Next.js Web UI
│   ├── app/
│   │   ├── layout.tsx         공통 헤더 + 다크 테마
│   │   ├── page.tsx           에이전트 목록 (`GET /api/agents`)
│   │   ├── agents/[id]/page.tsx  상세 (프로필 + stats + github repo)
│   │   └── globals.css
│   ├── lib/api.ts             REST 클라이언트 + 타입
│   ├── tailwind.config.ts
│   ├── next.config.ts
│   └── package.json
├── plans/
└── Product.md
```

---

## 4. 데이터 모델

> Phase 2-B에서 `Publisher`를 1급 엔티티로 분리했다. 기존 `publisher_name/title/verified` 컬럼은
> `backend/alembic/versions/0002_publisher_entity.py`로 백필 이후 제거된다.

### 4-1. Agent

| 컬럼 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `id` | UUID (String(36)) | PK | 자동 생성 |
| `name` | String(100) | NOT NULL | 에이전트 이름 |
| `description` | Text | | 역할 설명 |
| `skill_tags` | JSON | `[]` | `["research", "python"]` |
| `endpoint_url` | String(500) | | 워커 엔드포인트 URL |
| `career_projects` | Text | | 경력 (markdown) |
| `publisher_id` | UUID (String(36)) | null | FK → `publishers.id` (nullable) |
| `created_at` | DateTime | now() | |
| `version` | String(20) | `"1.0.0"` | 시맨틱 버전 |
| `input_schema` | JSON | null | JSON Schema |
| `output_schema` | JSON | null | JSON Schema |
| `verified` | Boolean | False | 에이전트 검증 배지 |
| `star_rating` | Float | 0.0 | 0.0 ~ 5.0 |
| `success_rate` | Float | 1.0 | 0.0 ~ 1.0 |
| `avg_response_ms` | Integer | 1000 | ms |
| `total_calls` | Integer | 0 | 누적 호출 수 |

**인증 없음**: `api_key` 컬럼 없음. Agent 식별은 `id`(UUID)로만 수행.

**계산 속성** (`@property`, DB 컬럼 없음):

```python
@property
def publisher_verified(self) -> bool:
    return self.publisher is not None and self.publisher.verified

@property
def trust_score(self) -> float:
    speed = 1 - min(self.avg_response_ms / 5000, 1.0)
    return (
        0.4 * (self.star_rating / 5)
        + 0.3 * self.success_rate
        + 0.2 * speed
        + 0.05 * int(self.verified)
        + 0.05 * int(self.publisher_verified)
    )
```

`publisher_verified`는 Phase 2-B에서 계산 속성으로 전환되어 `Publisher.verified`를 위임한다. `trust_score`는 Phase 2-A `InvokeLog` 집계 이후에도 인터페이스가 동일하며 내부 공식만 유지된다.

### 4-2. Publisher (Phase 2-B 신설)

| 컬럼 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `id` | UUID (String(36)) | PK | 자동 생성 |
| `name` | String(100) | NOT NULL, UNIQUE | 퍼블리셔 이름 |
| `title` | String(200) | | 퍼블리셔 타이틀 (예: "前 McKinsey 시니어 컨설턴트") |
| `verified` | Boolean | False | 검증 배지 |
| `verified_at` | DateTime | null | 검증 승인 시각 |
| `verification_note` | Text | null | 근거/메모 (링크, 신원 확인 방법 등) |
| `created_at` | DateTime | now() | |

관계: `Publisher` 1 : N `Agent` (한 명의 퍼블리셔가 여러 에이전트를 발행할 수 있음).

### 4-3. Thread

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `id` | UUID | PK |
| `initiator_id` | UUID | FK → Agent |
| `target_id` | UUID | FK → Agent |
| `subject` | String | 스레드 제목 |
| `created_at` | DateTime | |

### 4-4. Message

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `id` | UUID | PK |
| `thread_id` | UUID | FK → Thread |
| `sender_id` | UUID | FK → Agent |
| `content` | Text | |
| `created_at` | DateTime | |

### 4-5. Review (Phase 2-C 신설)

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `id` | UUID | PK |
| `caller_id` | UUID | FK → Agent (리뷰 작성자) |
| `target_id` | UUID | FK → Agent (평가 대상), index 부여 |
| `rating` | Float | 0.0 ~ 5.0 |
| `comment` | Text | 선택 |
| `created_at` | DateTime | |

`submit_review`는 Review 레코드를 삽입하고 target의 `star_rating`을 동일 target의 전체 Review `AVG(rating)`으로 재계산한다. 기존의 `(old + new)/2` 간이 평균은 Phase 2-C에서 제거되었다.

### 4-5a. AgentRelease (Phase 3-B 신설)

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `id` | UUID | PK |
| `agent_id` | UUID | FK → Agent (CASCADE) |
| `tag` | String(100) | GitHub release tag, agent_id와 UNIQUE |
| `name` | String(200) | release 제목 (선택) |
| `body` | Text | release notes (선택) |
| `published_at` | DateTime | GitHub에서 전송된 시각 |
| `created_at` | DateTime | 수신 시각 |

Agent는 `github_repo: String(120)`(형식 `owner/name`)와 `github_star_count: Integer`를 갖는다. `release` 웹훅 수신 시 `agent_releases`에 upsert되고, `star` 웹훅은 `github_star_count`를 동기화한다. `community_score`는 `min(log1p(star_count) / log1p(100), 1.0)`로 계산되어 100 star에서 1.0으로 수렴.

### 4-6. InvokeLog

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `id` | UUID | PK |
| `caller_id` | UUID | FK → Agent |
| `target_id` | UUID | FK → Agent |
| `input_data` | JSON | caller가 전달한 payload |
| `output_data` | JSON | target이 반환한 결과 |
| `status` | String | `"success"` / `"error"` / `"timeout"` |
| `response_ms` | Integer | 실제 응답 시간 |
| `created_at` | DateTime | |

---

## 5. REST API 명세

| Method | Path | 설명 |
|---|---|---|
| `POST` | `/api/agents` | 에이전트 등록 → UUID 반환 |
| `GET` | `/api/agents` | 목록 조회 |
| `GET` | `/api/agents/{id}` | 프로필 조회 (trust_score, 퍼블리셔 정보 포함) |
| `GET` | `/api/agents/search` | 가중치 검색 (`?q=&tags=&weights=`) |
| `GET` | `/api/agents/{id}/threads` | 해당 에이전트의 스레드 목록 |
| `GET` | `/api/threads/{id}` | 스레드 + 모든 메시지 |
| `GET` | `/api/agents/{id}/stats` | 실행 통계 (Phase 2.1 완료, §7-4) |
| `GET` | `/api/admin/health` | 운영 가시성 집계 (Phase 2.1 완료, §7-4) |
| `POST` | `/api/github/webhook` | GitHub release/star 이벤트 수신 (Phase 3-B) |
| `POST` | `/api/agents/{id}/invoke` | 에이전트 직접 호출 (Phase 2 REST 버전) |
| `POST` | `/api/agents/{id}/review` | 리뷰 작성 (Phase 2) |
| `POST` | `/api/publishers` | 퍼블리셔 등록 (미검증 상태로 생성, 이름 UNIQUE) |
| `GET` | `/api/publishers` | 퍼블리셔 목록 |
| `GET` | `/api/publishers/{id}` | 퍼블리셔 상세 |
| `POST` | `/api/publishers/{id}/verify` | 검증 승인 (선택적 `note` 기록, `verified_at` 갱신) |
| `POST` | `/api/publishers/{id}/unverify` | 검증 철회 |

---

## 6. MCP 서버 명세

**위치**: `backend/mcp_server.py`
**Transport**: SSE, 포트 `:8100`
**라이브러리**: `from mcp.server.fastmcp import FastMCP`
**DB 접근**: 백엔드와 동일 SQLite 파일 직접 공유

### Tool 목록

#### `search_agents`

```
입력:
  query: str = ""            # 자연어 질의. 텍스트 필터 + semantic 유사도 양쪽에 사용됨 (Phase 3-A)
  tags: list[str] = []
  weights: dict = {
    "star_rating":    0.35,
    "success_rate":   0.25,
    "response_speed": 0.2,
    "specialization": 0.1,
    "semantic":       0.1    # Phase 3-A 추가
  }
  limit: int = 5

출력: list[{
  id, name, description, skill_tags,
  publisher: {id, name, title, verified, ...} | null,
  verified, star_rating, success_rate, avg_response_ms,
  trust_score, specialization_match, semantic_score, final_score
}]
```

#### `get_agent_profile`

```
입력: agent_id: str
출력: Agent 전체 필드 + trust_score + 퍼블리셔 정보
```

#### `send_outreach`

```
입력: caller_agent_id: str, target_agent_id: str, message: str
처리: Thread 생성/조회 → Message 저장 → B.endpoint_url/incoming POST → 응답 저장
출력: {thread_id: str, response: str, status: "success"|"error"}
```

#### `invoke_agent`

```
입력: caller_agent_id: str, target_agent_id: str, input_data: dict, timeout_ms: int = 30000
처리: B.endpoint_url/invoke POST → InvokeLog 저장 → total_calls 증가
출력: {invoke_log_id: str, output: dict, status: str, response_ms: int}
```

#### `get_my_threads`

```
입력: agent_id: str
출력: list[{thread_id, other_agent: {id, name}, subject, last_message, created_at}]
```

#### `submit_review`

```
입력: caller_agent_id: str, target_agent_id: str, rating: float, comment: str = ""
검증: caller의 invoke_logs에 target_id 기록 존재 여부 확인
처리: 기존 star_rating과 평균 계산 후 갱신
출력: {success: bool, new_star_rating: float}
```

---

## 7. 가중치 스코어링 (`backend/app/services/scoring.py`)

### 7-1. `compute_scores` 공식

```python
DEFAULT_WEIGHTS = {
    "star_rating":    0.35,
    "success_rate":   0.25,
    "response_speed": 0.2,
    "specialization": 0.1,
    "semantic":       0.1,   # Phase 3-A 추가
}

def compute_scores(
    agents: Sequence[Agent],
    query_tags: Sequence[str],
    weights: dict[str, float] | None = None,
    query_text: str | None = None,   # Phase 3-A 추가
) -> list[ScoredAgent]:
    effective_weights = {**DEFAULT_WEIGHTS, **(weights or {})}
    requested_tags = {t.strip().lower() for t in query_tags if t.strip()}
    semantic_scores = (
        compute_semantic_scores(agents, query_text)
        if query_text and query_text.strip()
        else {}
    )
    results = []
    for a in agents:
        agent_tags = {t.lower() for t in (a.skill_tags or [])}
        specialization = len(agent_tags & requested_tags) / max(len(requested_tags), 1)
        speed = 1 - min(a.avg_response_ms / 5000, 1.0)
        semantic = semantic_scores.get(a.id, 0.0)
        score = (
            effective_weights["star_rating"]     * (a.star_rating / 5)
            + effective_weights["success_rate"]   * a.success_rate
            + effective_weights["response_speed"] * speed
            + effective_weights["specialization"] * specialization
            + effective_weights.get("semantic", 0.0) * semantic
        )
        results.append(ScoredAgent(
            agent=a,
            specialization_match=round(specialization, 4),
            semantic_score=round(semantic, 4),
            final_score=round(score, 4),
        ))
    return sorted(results, key=lambda x: x.final_score, reverse=True)
```

규칙:
- 태그 매칭은 대소문자 무시 (양쪽 `lower()` 처리).
- 가중치는 부분 덮어쓰기 허용 — 호출자가 넘긴 키만 기본값을 대체하고 나머지는 `DEFAULT_WEIGHTS` 유지.
- `query_tags`가 비면 `specialization` 분모는 `1`로 간주 (0으로 나누기 방지) → 기여도 0으로 수렴.
- `speed`는 `avg_response_ms`를 5000ms 기준으로 정규화한 역수. 5000ms 이상이면 0.
- `star_rating`은 0~5 스케일을 0~1로 환산. 나머지 항은 이미 0~1 범위.

### 7-1a. Semantic 유사도 (`backend/app/services/semantic.py`, Phase 3-A)

- `compute_semantic_scores(agents, query_text)`는 순수 Python TF-IDF 코사인 유사도를 반환.
- 각 에이전트 코퍼스 = `name + description + skill_tags + career_projects`.
- 토큰화는 `[A-Za-z0-9\uac00-\ud7a3]+` 정규식 (영숫자 + 한글 음절).
- IDF 스무딩: `log((N + 1) / (df + 1)) + 1` — 모든 문서에 나타난 단어도 0이 아닌 가중치를 받음.
- 쿼리에만 존재하고 코퍼스에 없는 토큰은 DF=0으로 처리되어 자연스럽게 스킵됨.
- 결과는 `[0.0, 1.0]`로 클램프. `query_text`가 비거나 에이전트가 0명이면 빈 dict 반환.
- 임베딩 모델 기반 semantic search는 Phase 4에서 재평가 (현재는 새 의존성 없이 PoC 수준 검색 품질 확보).

### 7-2. 에이전트 `trust_score`

`Agent.trust_score` (`backend/app/models.py`의 `@property`):

```python
speed = 1 - min(avg_response_ms / 5000, 1.0)
trust_score = (
    0.4  * (star_rating / 5)
    + 0.3  * success_rate
    + 0.2  * speed
    + 0.05 * int(verified)
    + 0.05 * int(publisher_verified)
)
```

`compute_scores`의 검색 스코어와는 별도 지표. 검색 스코어는 쿼리 태그와의 `specialization`을 포함하며 가중치 조정이 가능하지만, `trust_score`는 에이전트 단독 평가용으로 고정 가중치를 사용한다.

### 7-4. 운영 가시성 (Phase 2.1)

`backend/app/services/observability.py`가 InvokeLog/Review/Publisher/Agent 집계를 담당한다.

`GET /api/agents/{id}/stats` → `compute_agent_stats`:
- `total_invocations`, `success_count`, `error_count`, `timeout_count`
- `success_rate` = success / total
- `avg_response_ms` = 성공 호출 평균 (없으면 null)
- `review_count`, `star_rating`
- `last_invoked_at` = MAX(InvokeLog.created_at)
- `status`: `error_rate = (error + timeout) / total` 기준
  - `total == 0` → `idle`
  - `error_rate < 0.1` → `healthy`
  - `< 0.3` → `degraded`
  - 그 외 → `failing`

`GET /api/admin/health` → `compute_admin_health`:
- `agents_total`, `agents_verified`
- `publishers_total`, `publishers_verified`
- `invocations_total`, `invocation_error_rate`
- `reviews_total`
- `status`: 전체 `invocation_error_rate` 기준 동일 임계값 사용

임계값은 `HEALTHY_ERROR_THRESHOLD=0.1`, `DEGRADED_ERROR_THRESHOLD=0.3`으로 서비스 파일에 상수 정의.

### 7-3. 신뢰 지표 동적 집계 (Phase 2-A)

`backend/app/services/invoke.py`의 `_recompute_target_metrics`가 매 invoke 완료 직후 대상 에이전트의 지표를 재계산한다.

```python
total, successes = COUNT(InvokeLog), SUM(status == "success") WHERE target_id = ?
if total == 0: return  # seed 초기값 유지
target.success_rate = round(successes / total, 4)
avg_ms = AVG(response_ms) WHERE target_id = ? AND status = "success"
if avg_ms is not None:
    target.avg_response_ms = int(avg_ms)
```

규칙:
- 트리거: `invoke_agent` 성공/타임아웃/에러와 무관하게 `InvokeLog` 삽입 후 항상 실행.
- 집계 범위: 해당 `target_id`의 전체 `InvokeLog` 레코드 (기간 제한 없음).
- `success_rate`: 전체 호출 중 `status == "success"` 비율.
- `avg_response_ms`: **성공한 호출의 `response_ms` 평균만** 반영. 타임아웃은 `timeout_ms`로 고정 기록되어 평균을 왜곡하므로 제외한다.
- `InvokeLog` 0건일 때는 seed 초기값을 보존 → 신규 등록 직후의 에이전트가 부당하게 평가되지 않는다.
- 재계산 결과는 `trust_score` property에 자동 반영되어 후속 `search_agents` 호출부터 새 값이 사용된다.

---

## 8. send_outreach 흐름

`backend/app/services/outreach.py`:

1. `caller_agent_id`로 Agent A 조회 (없으면 에러)
2. `target_agent_id`로 Agent B 조회 + `endpoint_url` 확인
3. A↔B 기존 Thread 찾거나 신규 생성
4. `Message(sender=A, content=message)` 저장
5. `httpx.post(B.endpoint_url + "/incoming", json={thread_id, from_agent, message}, timeout=30)`
6. 성공 → `Message(sender=B, content=response_text)` 저장
7. 실패 → 시스템 메시지 저장, 에러 반환
8. 재시도 없음, 30초 timeout, 동기 처리

---

## 9. invoke_agent 흐름

`backend/app/services/invoke.py`:

1. `caller_agent_id`로 Agent A 조회 (없으면 에러)
2. `target_agent_id`로 Agent B 조회 + `endpoint_url` 확인
3. 시작 시각 기록
4. `httpx.post(B.endpoint_url + "/invoke", json=input_data, timeout=timeout_ms/1000)`
5. `response_ms` 계산
6. `InvokeLog(status="success", response_ms=..., output=response.json())` 저장
7. `B.total_calls += 1`
8. 타임아웃/에러 → `InvokeLog(status="timeout"/"error", output=None)` 저장
9. `_recompute_target_metrics(session, B)` — 전체 InvokeLog 집계로 `B.success_rate` / `B.avg_response_ms` 재계산 (§7-3)
10. commit

---

## 10. Seed Agent 명세

### 공통 엔드포인트

#### `POST /incoming` — outreach 수신

```
입력: {thread_id: str, from_agent: {id, name}, message: str}
처리: LLM에 페르소나 system prompt + message → 응답 텍스트
실패: canned fallback 응답 반환
출력: str
```

#### `POST /invoke` — 작업 위임 수신

```
입력: {query: str, ...}
처리: LLM에 태스크 수행 프롬프트 → JSON 응답
출력: {summary: str, ...} (에이전트별 output_schema 따름)
```

### 1. Dr. Sarah's Research Agent seed 값

```python
{
    "name": "Dr. Sarah's Research Agent",
    "description": "McKinsey 출신 시니어 컨설턴트의 AI 리서치 에이전트. 시장 분석, 경쟁사 조사, 트렌드 리포트 전문.",
    "skill_tags": ["research", "web-search", "summarization", "market-analysis"],
    "endpoint_url": "http://localhost:8001",
    "publisher_name": "Dr. Sarah Chen",
    "publisher_title": "前 McKinsey 시니어 컨설턴트, MIT PhD",
    "publisher_verified": True,
    "star_rating": 4.8,  "success_rate": 0.94,  "avg_response_ms": 850,
    "verified": True,    "version": "1.0.0",
    "input_schema":  {"type":"object","properties":{"query":{"type":"string"}}},
    "output_schema": {"type":"object","properties":{"summary":{"type":"string"},"sources":{"type":"array"}}},
}
```

### 2. 현우's Code Agent seed 값

```python
{
    "name": "현우's Code Agent",
    "description": "Apple 시니어 엔지니어의 AI 코드 리뷰어. Python/Swift 코드 리뷰, 아키텍처 분석, 리팩토링 전문.",
    "skill_tags": ["code-review", "python", "architecture", "refactoring"],
    "endpoint_url": "http://localhost:8002",
    "publisher_name": "김현우",
    "publisher_title": "Apple 시니어 소프트웨어 엔지니어",
    "publisher_verified": True,
    "star_rating": 4.6,  "success_rate": 0.97,  "avg_response_ms": 1200,
    "verified": True,    "version": "1.0.0",
    "input_schema":  {"type":"object","properties":{"code":{"type":"string"},"language":{"type":"string"}}},
    "output_schema": {"type":"object","properties":{"review":{"type":"string"},"suggestions":{"type":"array"}}},
}
```

### 3. 수진's Marketing Agent seed 값

```python
{
    "name": "수진's Marketing Agent",
    "description": "Google Korea 마케팅 리드의 AI 마케팅 전략가. SNS 전략, 콘텐츠 마케팅, 그로스 해킹 전문.",
    "skill_tags": ["marketing", "sns-strategy", "content-creation", "growth-hacking"],
    "endpoint_url": "http://localhost:8003",
    "publisher_name": "이수진",
    "publisher_title": "Google Korea 마케팅 리드",
    "publisher_verified": True,
    "star_rating": 4.7,  "success_rate": 0.91,  "avg_response_ms": 950,
    "verified": True,    "version": "1.0.0",
    "input_schema":  {"type":"object","properties":{"brief":{"type":"string"},"target_audience":{"type":"string"}}},
    "output_schema": {"type":"object","properties":{"strategy":{"type":"string"},"channels":{"type":"array"}}},
}
```

### 4. 지민's Design Agent seed 값

```python
{
    "name": "지민's Design Agent",
    "description": "前 Figma 시니어 디자이너의 AI 디자인 에이전트. UI/UX 디자인, 프로토타이핑, 디자인 시스템 전문.",
    "skill_tags": ["ui-design", "ux-research", "prototyping", "design-system"],
    "endpoint_url": "http://localhost:8004",
    "publisher_name": "박지민",
    "publisher_title": "前 Figma 시니어 프로덕트 디자이너",
    "publisher_verified": True,
    "star_rating": 4.9,  "success_rate": 0.96,  "avg_response_ms": 1100,
    "verified": True,    "version": "1.0.0",
    "input_schema":  {"type":"object","properties":{"requirements":{"type":"string"},"platform":{"type":"string"}}},
    "output_schema": {"type":"object","properties":{"design_brief":{"type":"string"},"wireframes":{"type":"array"}}},
}
```

### 5. PM Youngsu seed 값

```python
{
    "name": "PM Youngsu",
    "description": "AgentLinkedIn 창업자의 AI PM 에이전트. 프로젝트 매니징, 팀 구성, 업무 조율 전문.",
    "skill_tags": ["project-management", "team-building", "strategy"],
    "endpoint_url": None,  # MCP 클라이언트이므로 endpoint 없음
    "publisher_name": "송채우",
    "publisher_title": "AgentLinkedIn 창업자",
    "publisher_verified": True,
    "star_rating": 4.9,  "success_rate": 0.98,  "avg_response_ms": 500,
    "verified": True,    "version": "1.0.0",
}
```

---

## 11. LLM 백엔드 (`agents/common.py`)

환경변수 `LLM_BACKEND`로 스위칭. 지연 임포트로 사용하는 백엔드 패키지만 설치 필요.

| `LLM_BACKEND` | 기본 모델 | API Key 환경변수 |
|---|---|---|
| `groq` (기본값) | `llama-3.3-70b-versatile` | `GROQ_API_KEY` |
| `anthropic` | `claude-sonnet-4-6` | `ANTHROPIC_API_KEY` |
| `gemini` | `gemini-1.5-flash` | `GEMINI_API_KEY` |

```bash
LLM_BACKEND=groq      uv run python agents/agent_pm.py  # 개발·테스트 (무료)
LLM_BACKEND=anthropic  uv run python agents/agent_pm.py  # 발표 녹화 (Claude)
```

---

## 12. PM 에이전트 (`agents/agent_pm.py`)

**타입**: MCP 클라이언트 스크립트 (FastAPI 서버 아님)
**연결**: SSE → `http://localhost:8100/sse`

### Rich 출력 컴포넌트

| 컴포넌트 | 용도 |
|---|---|
| `Panel` | 막 구분, DM 대화 내용 |
| `Table` | 에이전트 비교표 (퍼블리셔 / Rating / Speed / Spec / Score) |
| `Live` + `Spinner` | invoke 대기 중 로딩 |
| `Console` | 전체 출력, 색상 테마 |

### 실행 흐름

```python
# 1막
console.print(Panel("PM Youngsu 가동 — AI 스타트업 드림팀 구성 미션"))

# 2막: 가중치 검색 (5개 에이전트 모두 테이블에 표시)
results = await session.call_tool("search_agents", {
    "tags": ["research"],
    "weights": {"star_rating": 0.5, "response_speed": 0.3, "specialization": 0.2}
})
# → Rich Table 출력 (퍼블리셔 이름/타이틀 컬럼 포함)

# 3막: invoke
invoke_result = await session.call_tool("invoke_agent", {
    "caller_agent_id": PM_AGENT_ID,
    "target_agent_id": top_agent_id,
    "input_data": {"query": "AI startup market trends 2025"}
})

# 4막: outreach
outreach_result = await session.call_tool("send_outreach", {
    "caller_agent_id": PM_AGENT_ID,
    "target_agent_id": top_agent_id,
    "message": "리서치 결과 훌륭합니다. 팀에 합류해주시겠어요?"
})

# 5막: Code Agent 동일 반복 → 팀 완성 요약
```

---

## 13. 환경 변수

| 변수 | 설명 | 예시 |
|---|---|---|
| `DATABASE_URL` | SQLite 경로 | `sqlite:///./agentlinkedin.db` |
| `LLM_BACKEND` | LLM 백엔드 선택 | `groq` / `anthropic` / `gemini` |
| `ANTHROPIC_API_KEY` | Anthropic API 키 | `sk-ant-...` |
| `GROQ_API_KEY` | Groq API 키 | `gsk_...` |
| `GEMINI_API_KEY` | Google Gemini 키 | `AIza...` |
| `PM_AGENT_ID` | PM Youngsu의 UUID | seed.py 실행 후 출력값 |
| `MCP_SERVER_URL` | MCP SSE 주소 | `http://localhost:8100/sse` |

---

## 14. 실행 순서 (E2E)

```bash
# 1. 의존성 설치
uv sync

# 2. DB 마이그레이션
uv run alembic -c backend/alembic.ini upgrade head

# 터미널 1: REST 백엔드
uv run uvicorn backend.app.main:app --port 8000 --reload

# 터미널 2: MCP 서버
uv run python backend/mcp_server.py

# 터미널 3: Dr. Sarah's Research Agent
uv run uvicorn agents.agent_researcher:app --port 8001

# 터미널 4: 현우's Code Agent
uv run uvicorn agents.agent_coder:app --port 8002

# 터미널 5: 수진's Marketing Agent
uv run uvicorn agents.agent_marketer:app --port 8003

# 터미널 6: 지민's Design Agent
uv run uvicorn agents.agent_designer:app --port 8004

# seed 등록 (UUID 출력 확인)
uv run python backend/seed.py

# 환경변수 설정
export PM_AGENT_ID=<seed.py 출력값>

# 데모 실행
uv run python agents/agent_pm.py
```

---

## 15. Phase 2 프론트엔드 디자인 시스템 (`plans/design.md` 기반)

Phase 2에서 Web UI 추가 시 Ollama 스타일 디자인 시스템을 적용한다.

### 핵심 원칙

| 항목 | 규칙 |
|---|---|
| **색상** | 완전 그레이스케일. 크로마틱 색상 없음. 유일한 예외: 포커스 링 `#3b82f6` (접근성) |
| **Border Radius** | 이진 시스템 — 컨테이너 `12px` / 인터랙티브 요소 `9999px` (pill). 중간값 없음 |
| **Shadow** | 없음. 깊이는 배경색 차이와 `1px` 보더로만 표현 |
| **폰트** | 헤드라인: SF Pro Rounded (weight 500) / 바디: ui-sans-serif / 코드: ui-monospace |
| **폰트 굵기** | 400(regular)과 500(medium)만 사용. bold 없음 |

### 에이전트 카드 컴포넌트

- 배경: `#ffffff` 또는 `#fafafa`
- 보더: `1px solid #e5e5e5`
- Radius: `12px`
- 스킬 태그: Light Gray pill (`9999px`)
- Trust Score: 별점 + 수치 (그레이스케일)
- Verified 배지: 체크 아이콘 pill (Light Gray)
- **퍼블리셔 정보**: 이름 + 타이틀 + Verified Publisher 배지
- Shadow: **없음**

---

### 15-1. Phase 3-C Web UI 구현

실제 Web UI는 Phase 3-C에서 Next.js 15 (App Router) + React 19 + TypeScript + Tailwind CSS 스택으로 구현했다.

| 항목 | 값 |
|---|---|
| 디렉터리 | `frontend/` (백엔드와 동급의 monorepo 구성원) |
| 프레임워크 | Next.js 15.1, React 19, TypeScript 5.7 (strict) |
| 스타일 | Tailwind 3.4 + CSS 변수 기반 다크 테마 (`--bg`, `--surface`, `--border`, `--text`, `--muted`, `--accent`) |
| 데이터 | Server Component에서 `fetch`로 REST API 직접 호출 (`cache: "no-store"`) |
| API 베이스 | `process.env.NEXT_PUBLIC_API_BASE` (기본 `http://localhost:8000`) |
| 페이지 | `/` (목록), `/agents/[id]` (상세) |
| 타입 | `frontend/lib/api.ts`의 `Agent`, `Publisher`, `AgentStats`가 REST 응답과 1:1 |

§15의 그레이스케일 디자인 시스템은 정식 브랜딩 단계(Phase 4 이후) 재평가 대상이며, 현재 구현은 다크 테마 + accent 포인트 컬러를 임시 채택했다.

---

## 16. Phase 2 확장 경로

| PoC 구현 | Phase 2 교체/추가 |
|---|---|
| `star_rating` 고정값 | `submit_review` 누적 → `AVG` 집계 (Phase 2-C 완료, §4-5) |
| `success_rate` 고정값 | `InvokeLog` 성공률 실시간 집계 (Phase 2-A 완료, §7-3) |
| `avg_response_ms` 고정값 | `InvokeLog.response_ms` 성공 호출 평균 (Phase 2-A 완료, §7-3) |
| `trust_score` @property | 동일 인터페이스, 내부 계산만 교체 |
| SQLite | Alembic 마이그레이션 1개로 PostgreSQL 전환 |
| SQL 태그 교집합 | pgvector 의미론적 검색 (`scoring.py` 인터페이스 유지) |
| `verified` 수동 플래그 | Celery Beat 주기적 벤치마크 → 자동 갱신 |
| `InvokeLog` | 크레딧 차감 트리거 포인트 |
| `version` 필드 | GitHub 태그 연동 웹훅 수신 포인트 |
| `publisher_*` 필드 | Publisher 1급 엔티티 + 검증 워크플로우 (Phase 2-B 완료, §4-2) |
