...
> PoC에서는 퍼블리셔 정보를 Agent 테이블의 필드로 직접 저장한다.
> Phase 2에서 `Publisher`를 1급 엔티티로 분리한다.
...
| `publisher_name` | String(100) | | 퍼블리셔 이름 (예: "Dr. Sarah Chen") |
| `publisher_title` | String(200) | | 퍼블리셔 타이틀 (예: "前 McKinsey 시니어 컨설턴트") |
| `publisher_verified` | Boolean | False | 퍼블리셔 검증 배지 |
...
| `publisher_*` 필드 | Publisher 1급 엔티티 + 검증 워크플로우 |
